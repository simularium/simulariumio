...
from simulariumio import __version__
...
class Args(argparse.Namespace):
    ...
    def __parse(self):
        ...
        p = argparse.ArgumentParser(
            prog="upload_benchmark_resources",
            description=(
...
def upload_benchmark_resources(args: Args):
    # Try running the pipeline
    try:
        # Get test resources dir
        resources_dir = (Path(__file__).parent / "resources").resolve(strict=True)

        # Report which directory will be used for upload
        ...
        package_name = "simulariumio/benchmark"
        ...
        if args.dry_run:
            ...
            log.info(f"Completed package dry run. Result hash: {built}")
        ...
        # Upload
            ...
            if confirmation:
                pushed = package.push(
                    package_name,
                    "s3://simulariumio-test-resources",
                    message=f"Test resources for `simulariumio` version: {__version__}.",
                )
...
def main():
    args = Args()
    upload_benchmark_resources(args)
