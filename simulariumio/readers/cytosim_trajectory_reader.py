...
class CytosimTrajectoryReader(TrajectoryReader):
    ...
    def parse_fiber_data_dimensions(self, fibers_lines: List[str]) -> List[List[int]]:
        """
    ...
    def parse_other_data_dimensions(self, lines: List[str]) -> List[int]:
        """
    ...
    def parse_data_dimensions(self, cytosim_data: Dict[str, List[str]]) -> Tuple[int]:
        """
        Parse Cytosim output files to get the total steps,
        maximum agents per timestep, and maximum subpoints per agent 
        """
        dimensions = self.parse_fiber_data_dimensions(cytosim_data["fibers"])
        for object_type in cytosim_data:
            if object_type != "fibers":
                dims = self.parse_other_data_dimensions(cytosim_data[object_type])
                if len(dimensions) > 0 and len(dims) != len(dimensions):
    ...
    def parse_fibers(
        self, fibers_lines: List[str], scale_factor: float, result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def parse_others(
        self, data_lines: List[str], scale_factor: float, result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Parse a Cytosim output file containing objects other than fibers 
    ...
    def read(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        simularium_data = {}
        (totalSteps, max_agents, max_subpoints) = self.parse_data_dimensions(cytosim_data)
        ...
        agent_data = self.parse_fibers(cytosim_data["fibers"], scale, agent_data)
        for object_type in data["data"]:
            agent_data = self.parse_others(cytosim_data[object_type], scale, agent_data)
